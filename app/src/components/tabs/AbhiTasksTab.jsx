...
const API = import.meta.env.VITE_API_URL || 'https://openclaw.boldbusiness.com/api-server';
...
function fmt$(n) {
  const v = Number(n) || 0;
  return v < 0.01 ? '<$0.01' : `$${v.toFixed(4)}`;
}
...
function TaskRow({ task, expanded, onToggle }) {
  ...
  return (
    <div style={{
      ...
      >
        {/* Priority */}
        <span title={`Priority: ${task.priority}`} style={{ fontSize: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontSize: 14, fontWeight: 600, color: '#e8eaff',
        ...
        <div style={{ textAlign: 'right', minWidth: 70 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#06E5EC' }}>
            {fmt$(task.costUsd)}
          </div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>
            {fmtTokens(task.totalTokens)} tok
          </div>
        </div>
      ...
      {expanded && (
        <div style={{
          ...
          <div style={{
            ...
            marginBottom: task.rawMessage ? 12 : 0,
          }}>
...
// ── Main tab ──────────────────────────────────────────────────────────────────
export default function AbhiTasksTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `${API}/api/abhi/tasks?days=${days}&status=${statusFilter}&limit=300`,
    ...
    <div style={{ padding: '24px 28px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#e8eaff' }}>
          📋 Abhi's Task Assignments
        </h2>
        <p style={{ margin: '6px 0 0', fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>
          Every task Abhinanda assigns to Laura — with real AI cost per task
        </p>
      </div>

      {/* KPI row */}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 28 }}>
        <KpiCard icon="📋" label="Total Tasks"    value={summary.total || 0} sub={`Last ${days} days`} />
      ...
      {daily.length > 0 && (
        <div style={{
          ...
          <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.6)', marginBottom: 12 }}>
            Daily task activity (last {days} days)
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 60, overflowX: 'auto' }}>
            {[...daily].reverse().map(d => {
              const maxTasks = Math.max(...daily.map(x => x.tasks), 1);
              const h = Math.max(4, (d.tasks / maxTasks) * 56);
              return (
                <div key={d.day} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '1 0 20px' }} title={`${d.day}: ${d.tasks} tasks, $${Number(d.cost).toFixed(3)}`}>
                  <div style={{
                    width: '100%', height: h,
                    background: 'linear-gradient(180deg,#06E5EC,#003BDF)',
                    borderRadius: '3px 3px 0 0', minWidth: 8,
                  }} />
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', marginTop: 3, whiteSpace: 'nowrap' }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16, alignItems: 'center' }}>
        ...
        {['all','pending','in_progress','completed'].map(s => (
          <button key={s} onClick={() => setStatus(s)} style={{
            ...
            border: '1px solid',
            borderColor: statusFilter === s ? '#06E5EC' : 'rgba(255,255,255,0.15)',
            background:  statusFilter === s ? 'rgba(6,229,236,0.12)' : 'rgba(255,255,255,0.04)',
            color: statusFilter === s ? '#06E5EC' : 'rgba(255,255,255,0.55)',
        ...
        <select
          ...
          style={{
            ...
            color: '#e8eaff', fontSize: 12, cursor: 'pointer',
          }}
        >
          {[7,14,30,60,90].map(d => (
            <option key={d} value={d} style={{ background: '#1a1e3a' }}>Last {d}d</option>
          ))}
        </select>
        ...
        <button
          ...
          style={{
            ...
            background: 'rgba(6,229,236,0.08)', color: '#06E5EC',
            cursor: 'pointer',
          }}
        >
          ↻ Refresh
      ...
      {!loading && !error && filtered.length === 0 && (
        <div style={{
          ...
          <div style={{ fontSize: 12, marginTop: 6 }}>
            Tasks are logged automatically when Abhi sends me a request in Google Chat.
          </div>
        </div>
      )}
