        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginTop: 2, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            ...
            {act.requestedBy && act.requestedBy !== 'System' && (
              <span style={{ color: '#8B5CF6' }}>by {act.requestedBy}</span>
            )}
            {act.costUsd > 0 && (
...
// ── Main component ────────────────────────────────────────────────────────────
export default function DarrenActivitiesTab() {
  ...
  const loadActivities = useCallback(async (d, cat) => {
    ...
    try {
      ...
      const r = await fetch(`/api/darren/activities?${qs}`, { headers: authHeaders() });
  ...
  const loadCosts = useCallback(async (d) => {
    setCostLoad(true);
    try {
      const r = await fetch(`/api/darren/db-costs?days=${d}`, { headers: authHeaders() });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      const data = await r.json();
      setCosts(data);
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 20, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: '#e8eaff' }}>
            📊 Darren — Activities & Costs
          </h2>
          <p style={{ margin: '5px 0 0', fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>
            Live from <strong style={{ color: 'rgba(255,255,255,0.6)' }}>bb_agents RDS</strong> · agent_activities · darren-abhi-agent
          </p>
        </div>
        {/* Day range selector */}
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          {[1, 3, 7, 14, 30].map(d => (
            <button key={d} onClick={() => setDays(d)} style={{
              ...
              border: '1px solid',
              borderColor: days === d ? '#8B5CF6' : 'rgba(255,255,255,0.12)',
              background:  days === d ? 'rgba(139,92,246,0.15)' : 'rgba(255,255,255,0.03)',
              color:       days === d ? '#c084fc' : 'rgba(255,255,255,0.4)',
              cursor: 'pointer',
            }}>
              {d === 1 ? 'Today' : `${d}d`}
            </button>
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden' }}>
          ...
          {!loading && acts.length > 0 && (
            <div style={{ padding: '10px 14px', fontSize: 11, color: 'rgba(255,255,255,0.2)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
              {acts.length} activities · source: bb_agents.agent_activities · agent_id: darren-abhi-agent
            </div>
          )}
        </div>
      )}
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden' }}>
              ...
              {[...costs.byDay].sort((a, b) => b.day > a.day ? 1 : -1).map(r => (
                <div key={r.day} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 80px 100px', gap: 8, padding: '9px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: 12 }}>
                  ...
                  <span style={{ textAlign: 'right', color: '#8B5CF6', fontWeight: 700 }}>{r.activity_count}</span>
              ...
              {/* Total row */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 80px 100px', gap: 8, padding: '9px 16px', background: 'rgba(139,92,246,0.06)', borderTop: '1px solid rgba(139,92,246,0.15)', fontSize: 12, fontWeight: 700 }}>
                <span style={{ color: 'rgba(255,255,255,0.5)' }}>Total ({DAYS_LABEL})</span>
                <span style={{ textAlign: 'right', color: '#8B5CF6' }}>{t.total_activities || 0}</span>
      ...
      {/* ── Footer ──────────────────────────────────────────────────────────── */}
      <div style={{ marginTop: 20, padding: '10px 14px', borderRadius: 8, background: 'rgba(139,92,246,0.04)', border: '1px solid rgba(139,92,246,0.12)', fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>
        🤖 Darren Stuart (darren-abhi-agent) · Data source: bb_agents RDS agent_activities table · Excludes cron-log and auto-tracker noise
      </div>
    </div>
  );
}
